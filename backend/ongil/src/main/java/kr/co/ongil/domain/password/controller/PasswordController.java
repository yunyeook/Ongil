...
public class PasswordController {
    ...
    @PostMapping("/reset")
    @Operation(summary = "비밀번호 재설정 (비밀번호 분실)", description = "전화번호 인증을 통해 비밀번호를 재설정합니다.")
    public ApiResponse<String> resetPasswordWithToken(@Valid @RequestBody PasswordResetWithTokenRequest request) {

        passwordService.resetPasswordWithToken(request);
        return ApiResponse.success(ResponseMessage.PASSWORD_RESET_WITH_TOKEN_SUCCESS);
    }
